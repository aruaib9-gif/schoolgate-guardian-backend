...
function decorate(school, counts, i) {
...
export async function listSchoolsWithAggregates() {
  ...
  return schools.map((s, i) => decorate(s, counts, i));
}
...
export function getKpisFrom(schools, pendingInvites) {
  ...
  const mrr = active.reduce((t, s) => t + planPrice(s.plan), 0);
  return {
...
export async function buildSeries() {
  ...
  const schools = await prisma.school.findMany({
    select: { created_date: true, status: true, subscription_plan: true, students: true, staff: true },
  });
  ...
  const revenue = windows.map((w) =>
    schools.filter((s) => s.created_date < w.end && s.status === 'active').reduce((t, s) => t + planPrice(s.subscription_plan), 0)
  );
